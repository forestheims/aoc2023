...
# ╔═╡ c0487175-2db6-49b2-97cf-835f857aa9df
function replace_keys_with_values(input_string, dictionary)
	output_string = input_string

	for (key, value) in dictionary
		if occursin(key, output_string)
			output_string = replace(output_string, key => value)
		end
	end
...
# ╔═╡ 3da259f9-9972-42e9-b695-82e6a1e3b4ce
...
# ╔═╡ 6e0ce050-9038-11ee-0fed-6bfe6ec4b2ba
example = "gsjgklneight6zqfz
...
# ╔═╡ fe3d1348-c51d-4f12-8465-085c61d8d4f4
y = split( example , "\n");

# ╔═╡ f11a35aa-0235-4cbf-be5b-8a6e31263096
begin
	q = []
	for x in y
		z = split(x, "")
		...
		for j in z
			i = i * j
			i = replace_keys_with_values(i, number_dictionary)
		end
		push!(q, i)
	end
	println(q)
...
# ╔═╡ 7f9e2dc2-01ec-4637-9d43-a5818d66e871
begin
	number_array = []
	qrx = for rx in q
		xrq = replace(rx, r"\D" => "")
		if (length(xrq) == 1)
			push!(number_array, parse(Int, xrq*xrq))
		elseif (length(xrq) == 2)
			push!(number_array, parse(Int, xrq))
		else
			first = xrq[1]
			last = xrq[end]
			push!(number_array, parse(Int, first*last))
		end
	end
		println(number_array)
end

# ╔═╡ eb2f6a88-f484-44f9-8fb5-521c72159ffc
total = sum(number_array)

# ╔═╡ 18cd13f5-2efe-4f15-9ef3-8119aab8499b
for x in y
	println(x)
end
